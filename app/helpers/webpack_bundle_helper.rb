...
module WebpackBundleHelper
  ...
  def asset_server
    port = Rails.env === "development" ? "3000" : "3035"
  ...
  def dev_manifest
    OpenURI.open_uri("#{asset_server}/assets/manifest.json").read
  end
  ...
  def manifest
    return @manifest ||= JSON.parse(prod_manifest) if Rails.env.production?
